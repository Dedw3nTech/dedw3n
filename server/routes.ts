...
This is an automated message from Dedw3n. Please do not reply to this email.`;
  ...
  app.post('/api/vendors/analytics/email', unifiedIsAuthenticated, async (req: Request, res: Response) => {
    try {
      const userId = (req.user as any)?.id;
      ...
      const userName = user[0].name || user[0].username || 'Vendor';
      ...
      const selectedLabel = timeRangeLabel[timeRange] || 'Last 30 Days';
      ...
      const emailHtml = `
        ...
        <head>
          <style>
            ...
            .metric { background: white; padding: 20px; margin: 15px 0; border-radius: 8px; border-left: 4px solid #4f46e5; }
        ...
        <body>
          <div class="container">
            ...
            <div class="content">
              <p>Hello ${userName},</p>
              <p>Here's your sales analytics summary:</p>
              
              <div class="metric">
                <div class="metric-label">Total Revenue</div>
                <div class="metric-value">£${Number(metrics.totalRevenue).toFixed(2)}</div>
              </div>
              
              <div class="metric">
                <div class="metric-label">Total Orders</div>
                <div class="metric-value">${Number(metrics.totalOrders)}</div>
              </div>
              
              <div class="metric">
                <div class="metric-label">Total Products</div>
                <div class="metric-value">${Number(metrics.totalProducts)}</div>
              </div>
      ...
      const emailText = `
Sales Analytics Report - ${selectedLabel}

Hello ${userName},

Here's your sales analytics summary:

Total Revenue: £${Number(metrics.totalRevenue).toFixed(2)}
Total Orders: ${Number(metrics.totalOrders)}
Total Products: ${Number(metrics.totalProducts)}
...
Log in to your dashboard for detailed analytics and insights.
      ...
      const emailSent = await sendEmail({
        to: userEmail,
        from: process.env.BREVO_SMTP_USER || 'noreply@dedw3n.com',
        subject: `Sales Analytics Report - ${selectedLabel}`,
        text: emailText,
        html: emailHtml
      });

      if (emailSent) {
        console.log(`[ANALYTICS] Report emailed successfully to ${userEmail}`);
        return res.status(200).json({ 
          success: true, 
          message: `Analytics report sent to ${userEmail}`
        });
      } else {
        console.error('[ANALYTICS] Failed to send email');
        return res.status(500).json({ error: 'Failed to send email' });
      }
    } catch (error) {
      console.error('[ANALYTICS] Error sending analytics email:', error);

#!/usr/bin/env python3
"""
Micro batch processing for steady progress
"""
import pandas as pd
import psycopg2
import os
...
print(f"Starting: {start_count:,} cities")

# Continue from row 53,000 (where we left off)
start_row = 53000
batch_size = 500  # Even smaller batches
max_batches = 30  # Process 15,000 rows
...
for batch in range(max_batches):
    ...
    if cities_data:
        ...
        print(f"{batch+1:2d}/30: +{len(cities_data):3d} cities (rows {row_start:,}-{row_end:,})")
...
print(f"Added: {total_added:,} cities")
